...
public class CampeonatoController {
  ...
  @GetMapping
  @ApiOperation(value = "Gets all campeonatos", note = "Returns a list of all campeonatos")
  ...
  @ApiOperation(
      ...
      note = "Creates a new campeonato with the provided details")
  ...
  @ApiOperation(
      ...
      note = "Updates the details of an existing campeonato")
  ...
  @DeleteMapping("{id}")
  @ApiOperation(value = "Deletes campeonato by id", note = "Deletes campeonato with given id")
  public ResponseEntity<CampeonatoDto> removerCampeonato(@PathVariable("id") Long id) {
    ...
    if (campeonato == null) return ResponseEntity.notFound().build();
    if (campeonato.getId() == -1) return ResponseEntity.status(HttpStatus.CONFLICT).body(null);
    return ResponseEntity.of(campeonato);
  }
}
